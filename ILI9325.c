...
/*	UART_sendData(2, (readValue & 0xff00) >> 8);
	UART_sendData(2, (readValue & 0x00ff));*/
...
void LCD_StringInt(uint16_t PosX, uint16_t PosY, uint16_t value, uint8_t alignRight) {
	...
	if (value == 0) {
		if (alignRight) {
			LCD_PutChar(PosX, PosY + (0*18), ' ');
			LCD_PutChar(PosX, PosY + (1*18), ' ');
			LCD_PutChar(PosX, PosY + (2*18), ' ');
			LCD_PutChar(PosX, PosY + (3*18), ' ');
